...
int sched_set_bucket()
{
	...
	// printf("SCHED sched_set_bucket message m1i1 %d, m1i2 %d\n", m_in.m_m1.m1i1, m_in.m_m1.m1i2);


	int arg_who = m_in.m_m1.m1i1;
	if ((rmp = find_proc(arg_who)) == NULL)
			return(ESRCH);
	int bucket = m_in.m_m1.m1i2;
	// printf("SCHED sched_set_bucket, called by : %d , bucket set to %d\n", rmp->mp_pid, bucket);
	...
	// TODO zmienić to
	m.m_pm_sched_scheduling_set_nice.endpoint	= rmp->mp_endpoint;
	m.m_pm_sched_scheduling_set_nice.maxprio	= bucket;
